...
if [ "$ELASTICSEARCH_START" -ne "1" ]; then
  ...
  counter=0
  CLUSTER_NAME=
  while [ -z "$CLUSTER_NAME" -a $counter -lt 300 ]; do
    sleep 1
    ((counter++))
    CLUSTER_NAME=$(curl localhost:9200/_cat/health?h=cluster 2> /dev/null | tr -d '[:space:]')
    echo "Waiting for Elasticsearch cluster to respond ($counter/300)"
  done
...
# TODO
# RUN THE PROGRAM TO INSERT THE DATA
...
curl -XPUT 'localhost:9200/.kibana/_settings' -d '{ "index" : { "number_of_replicas" : 0 } }'
curl -XPUT 'localhost:9200/template/_settings' -d '{ "index" : { "number_of_replicas" : 0 } }'
